...
sequenceDiagram
    ...
    Tools-->>LLM: email incl. injection — as DATA only

    LLM->>Host: function_call analyze_untrusted_content(...)
    Host->>Gate: analyze? → ✅ ALLOWED (read)
    Host->>Ana: email text + fixed schema
    Ana-->>Host: filled schema (claims, embedded instructions, ...)
    Note over Host: validateAnalysis() — host validates & stores

    LLM->>Host: function_call search_customer_record(INV-2041)
    Host->>Tools: CRM lookup → 1 payment of 129 EUR
    Tools-->>LLM: CRM record — as DATA

    rect rgb(255, 230, 230)
    LLM->>Host: function_call issue_refund(INV-2041, 129 EUR)
    Host->>Gate: issue_refund?
    Note over Gate: rule 1 pass · rule 2 HIT<br/>(implied_tool in embedded instructions,<br/>not requested by customer)
    Gate->>Ledger: blocked (reason logged)
    Gate-->>Host: ⛔ BLOCKED — tool never runs
...
**Steps 11–12 — the model fills the fixed schema (2nd, isolated LLM call):**
...
**Steps 16–20 — the gate blocks (deterministic host code, no LLM):**
